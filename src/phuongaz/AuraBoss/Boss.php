...
Class Boss extends PluginBase implements Listener
{
	...
	public function getDrops($id){
		...
		foreach($items as $item){
			...
			if(!isset($ex[2])) $ex[2] = 0;
			$drops[] = Item::get($ex[0], $ex[1], $ex[2]);
		}
		return $drops;
	}

	public function chance($succes = 100){
	...
	public function getCustomDrops($id){
		...
		foreach(array_keys($items) as $sitem){
			...
			$drops["wings"] = [];
			if(isset($sitem["wings"])){
				$drops["wings"] = [$sitem["wings"]["type"] => $sitem["wings"]["chance"]];					
			}else{
				$ex = explode(":", $sitem["id"]);
	...
	public function spawnBoss($id){
		...
		//ChunkRegion::onChunkGenerated($player->getLevel(), $ent->x >> 4, $ent->z >> 4, function() use($ent){
			$ent->spawnToAll();			
        //});
	...
	public function spawnAllBoss(){
		...
		Server::getInstance()->broadcastMessage(TF::BOLD. TF::BLUE. "§r§l⩗§c Tất Cả Các Quái Thú Đã Xuất Hiện, Các Tù Nhân Hãy Tiêu Diệt Chúng!");
	}
